...
def equip_chinese_ner_with_lexicon(datasets,
    ...
    def get_skip_path(chars, w_trie):
        ...
        return result
    from lattice.utils_ import Trie
    ...
    for each in datasets:
        print(each)
        temp = partial(get_skip_path,w_trie=w_trie)

    for v in datasets.items():
        # 添加lexicons的index起始和终止
        v.apply_field(partial(get_skip_path,w_trie=w_trie),'raw_chars','lexicons')
        v.apply_field(copy.copy, 'raw_chars','raw_chars')
    ...
    if number_normalized == 1:
        for k,v in datasets.items():
            v.apply_field(normalize_char,'raw_chars','chars')
        vocabs['char'] = Vocabulary()
        vocabs['char'].from_dataset(datasets['train'], field_name='chars',
                                no_create_entry_dataset=[datasets['dev'], datasets['test']])
    ...
    vocabs['raw_chars'].index_dataset(* (datasets.values()),
