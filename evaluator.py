...
from cival_wospert.entities_rules import Document, Dataset, EntityType
from cival_wospert.input_reader import JsonInputReader
from cival_wospert.opt import jinja2
...
class Evaluator:
    ...
    def compute_scores(self):
        ...
        ner_eval = self._score_entity(gt, pred, print_results=True)
    ...
    def _convert_gt(self, docs: List[Document]):
        for doc in docs:
            ...
            sample_gt_entities = [entity.as_tuple_evalute() for entity in gt_entities]
    ...
    def _score_entity(self, gt: List[List[Tuple]], pred: List[List[Tuple]], print_results: bool = False):
        assert len(gt) == len(pred)
    ...
    def _print_results(self, per_type: List, micro: List, macro: List, types: List):
        ...
        print(results_str)
        log_file_path = 'E:\\NLP\\NER_RE\\spert-master\\scripts\\data\\save\\rules_datasets_train\\tempDir\\final_model\\log_file'
        with open(log_file_path,'a',encoding = 'utf-8') as f:
            self.eval_count += 1
            f.writelines('\n')
            f.writelines('============================================' + str(self.eval_count))
            f.writelines('\n')
